# This Makefile is for the Acme::DonMartin extension to perl.
#
...
#     ABSTRACT_FROM => q[DonMartin.pm]
#     AUTHOR => q[David Landgren]
#     LICENSE => q[perl]
#     NAME => q[Acme::DonMartin]
#     PREREQ_PM => { Compress::Zlib=>q[0] }
#     VERSION_FROM => q[DonMartin.pm]
...
TO_INST_PM = DonMartin.pm

PM_TO_BLIB = DonMartin.pm \
	$(INST_LIB)/Acme/DonMartin.pm
...
DISTNAME = Acme-DonMartin
DISTVNAME = Acme-DonMartin-0.08
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  DonMartin.pm $(INST_MAN3DIR)/Acme::DonMartin.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                Acme-DonMartin' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.08' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            For programs that are easy to dictate over the telephone' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>For programs that are easy to dictate over the telephone</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  DonMartin.pm $(INST_LIB)/Acme/DonMartin.pm 
